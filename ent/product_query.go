...
func (pq *ProductQuery) QueryOrderProducts() *OrderProductsQuery {
	query := (&OrderProductsClient{config: pq.config}).Query()
...
func (pq *ProductQuery) WithOrderProducts(opts ...func(*OrderProductsQuery)) *ProductQuery {
	query := (&OrderProductsClient{config: pq.config}).Query()
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withOrderProducts; query != nil {
		if err := pq.loadOrderProducts(ctx, query, nodes,
			func(n *Product) { n.Edges.OrderProducts = []*OrderProducts{} },
			func(n *Product, e *OrderProducts) { n.Edges.OrderProducts = append(n.Edges.OrderProducts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadOrderProducts(ctx context.Context, query *OrderProductsQuery, nodes []*Product, init func(*Product), assign func(*Product, *OrderProducts)) error {
	...
	query.withFKs = true
	query.Where(predicate.OrderProducts(func(s *sql.Selector) {
	...
	for _, n := range neighbors {
		fk := n.product_order_products
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_order_products" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_order_products" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
