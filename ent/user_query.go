...
func (uq *UserQuery) QueryBaskets() *BasketQuery {
	query := (&BasketClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBaskets: uq.withBaskets.Clone(),
...
func (uq *UserQuery) WithBaskets(opts ...func(*BasketQuery)) *UserQuery {
	query := (&BasketClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withBaskets = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withBaskets; query != nil {
		if err := uq.loadBaskets(ctx, query, nodes,
			func(n *User) { n.Edges.Baskets = []*Basket{} },
			func(n *User, e *Basket) { n.Edges.Baskets = append(n.Edges.Baskets, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadBaskets(ctx context.Context, query *BasketQuery, nodes []*User, init func(*User), assign func(*User, *Basket)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(basket.FieldUserID)
	}
	query.Where(predicate.Basket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BasketsColumn), fks...))
	}))
