...
func (oq *OrderQuery) QueryOrderProducts() *OrderProductsQuery {
	query := (&OrderProductsClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithOrderProducts(opts ...func(*OrderProductsQuery)) *OrderQuery {
	query := (&OrderProductsClient{config: oq.config}).Query()
...
//
//	client.Order.Query().
//		GroupBy(order.FieldPaymentType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (oq *OrderQuery) loadOrderProducts(ctx context.Context, query *OrderProductsQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderProducts)) error {
	...
	for _, n := range neighbors {
		fk := n.order_order_products
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_order_products" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_order_products" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
